...
import com.example.e_commerceapplication.admin.adapter.UserPaymentsHistoryAdapter;
...
import com.example.e_commerceapplication.models.product.Payment;
...
public class UserHistoryActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dataLayer.getDatabaseReference().child(ADMIN_PATH)
        .get()
        .addOnCompleteListener(task -> task.getResult().getChildren().forEach(child -> {
                User user = child.getValue(User.class);
            assert user != null;

            dataLayer.getFireStore().collection(USER_PAYMENTS)
                    .document(user.getUserID())
                    .collection(USER)
                    .get()
                    .addOnCompleteListener(pay -> pay.getResult().getDocuments().forEach(task1 -> {
